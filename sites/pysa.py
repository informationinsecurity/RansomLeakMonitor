...
def scrape(ta_url,ta,proxies,timestamp, mydb,writedb,screenshot,workingdir,tbb_dir,imgbb_key,imgbb_url):
    ...
    #working pulling victims of pysa
    # get max page number
    victim_list = soup.find_all("div", class_="page-header")
    last_published_dt = None

    for victims in victim_list:
        victim = victims.find_all("a")[0].text.strip()
        date = victims.find_all("span")[1].text.strip()
        victim = victim.replace("'","")
        victim_links = victims.find("a", href=True)
        victim_links = victim_links['href']
        victim_links = ta_url + victim_links
        victim_links = victim_links.replace("partners.html","")
        print(victim)
        print(date)
        print(victim_links)
        victim_count += 1
