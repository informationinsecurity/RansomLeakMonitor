...
def scrape(ta_url,ta,proxies,timestamp,mydb,writedb,screenshot,workingdir,tbb_dir,imgbb_key,imgbb_url,tor_ff_path,tor_control_pass,ff_binary,ff_profile):
    ...
    #working pulling victims of mosses_staff
    div = soup.find_all("div", class_="elementskit-post-body")
    ...
    for links in div:
        for victim_links in links.find_all("a", href=True):
            ...
            vicdiv = vicsoup.find("h2", class_="elementor-heading-title elementor-size-xl")
            victim = vicdiv.text
            victim = victim.strip()
            print(victim)
            date = timestamp
