...
#working pulling victims of Bonaci
...
def scrape(ta_url,ta,proxies,timestamp,mydb,writedb,screenshot,workingdir,tbb_dir,imgbb_key,imgbb_url,tor_ff_path,tor_control_pass,ff_binary,ff_profile):
    ...
    div = soup.find_all("div", class_="col-md-8")
    ...
    for links in div:
        for victim_links in links.find_all("a", href=True):
            ...
            vicdiv = vicsoup.find("div", class_="blog-title")
            victim = vicdiv.text
            victim = victim.strip()
            print(victim)
            date = timestamp
    ...
    url_catalog = ta_url + "/catalog"
    page = requests.get(url_catalog, timeout=30, proxies=proxies, headers=headers)
    soup = BeautifulSoup(page.content, 'html.parser')
    
    #now to scrape the data auction piece of their site
    div = soup.find_all("div", class_="col-md-8")
    #pulls links for each victim post - otherwise names are truncated with ...
    for links in div:
        for victim_links in links.find_all("a", href=True):
