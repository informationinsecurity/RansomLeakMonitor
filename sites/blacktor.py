...
def scrape(ta_url,ta,proxies,timestamp,mydb,writedb,screenshot,workingdir,tbb_dir,imgbb_key,imgbb_url,tor_ff_path,tor_control_pass,ff_binary,ff_profile):
    ...
    username = "bl@ckt0r"
    password = "bl@ckt0r"
    base_url = "http://bl4cktorpms2gybrcyt52aakcxt6yn37byb65uama5cimhifcscnqkid.onion"
    #working pulling victims of synack
    headers = {'User-Agent':'Mozilla/5.0 (Windows NT 10.0; WOW64; rv:50.0) Gecko/20100101 Firefox/50.0'}
    page = requests.get(ta_url, timeout=30, proxies=proxies, headers=headers, auth=(username, password))
    ...
    for links in div:
        for victim_links in links.find_all("a", href=True):
            ...
            victim = victim_links
            victim_links = base_url + "/" + victim_links
            print("Pulling Victim from Page" + victim_links)
            victim = victim.replace("breach/breach-", "")
            victim = victim.replace(".txt", "")
            print(victim)
            date = timestamp
            print("Date is: ")
            print(date)
