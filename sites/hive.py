...
def scrape(ta_url,ta,proxies,timestamp, mydb,writedb,screenshot,workingdir,tbb_dir,imgbb_key,imgbb_url):
    ...
    #working pulling victims of Hive
    headers = {'User-Agent':'Mozilla/5.0 (Windows NT 10.0; WOW64; rv:50.0) Gecko/20100101 Firefox/50.0'} 
    page = requests.get(ta_url, timeout=30, proxies=proxies, headers=headers)
    soup = BeautifulSoup(page.content, 'html.parser')
    #working pulling victims of Xing
    site_json = json.loads(soup.text)
    #hive scrapes via API so victim screenshots aren't easy
    victim_links = "http://hiveleakdbtnp76ulyhi52eag6c6tyc3xw7ez7iqy6wc34gd2nekazyd.onion"
    for victim in site_json:
       victim = victim["title"]
    ...
    print("Total Victim Count for " + ta + ": " + victim_count)
